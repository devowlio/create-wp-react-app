 ...
 * @param slug
 */
function modifyRootGitLabCiInclude(action, cwd, slug) {
    utils_1.logProgress(
        "Modify root .gitlab-ci.yml and " + action + " include " + chalk_1.default.underline(slug) + "..."
    );
    var path = path_1.resolve(cwd, ".gitlab-ci.yml");
    var content = fs_1.readFileSync(path, { encoding: utils_1.DEFAULT_ENCODING });
    content = content.replace(/# create-wp-react-app -->\n(.*)\n(\s*)# <-- create-wp-react-app/gms, function(
        match,
        inner,
        spaceBefore
    ) {
        ...
        if (action === "remove") {
            newLines = newLines.filter(function(line) {
                return line.indexOf("- /plugins/" + slug + "/") === -1;
            });
        } else {
