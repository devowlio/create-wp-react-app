...
use MatthiasWeb\WPRJSS as root;
...
abstract class Core extends Base {
    ...
    public function i18n() {
        load_plugin_textdomain(
            WPRJSS_TD,
            false,
            dirname(plugin_basename(WPRJSS_FILE)) . $this->getPluginData('DomainPath')
        );
    }
    ...
    public function getPluginData($key = null) {
        ...
        $data = isset($this->plugin_data)
            ? $this->plugin_data
            : ($this->plugin_data = get_plugin_data(WPRJSS_FILE, true, false));
