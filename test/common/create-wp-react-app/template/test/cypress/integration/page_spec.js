...
beforeEach("Automatically login to WordPress dashboard as admin user", function() {
    //cy.exec("yarn root:run db:snapshot-import"); // You can also reset the database before each test
    cy.exec(`yarn --silent root:run wp-cli "wp core version"`); // Just to check if WP is generelly available
    cy.visit("/wp-login.php?autologin=wordpress");
    cy.url().should("contain", "wp-admin");
});

describe("WP Admin Page", function() {
    const componentId = pkg.slug + "-component";
